...
public class HidBridge {
    private FunctionalInterface _txt;
...
//	private HidBridgeLogSupporter _logSupporter = new HidBridgeLogSupporter();
			...
			while (true) {
				...
				synchronized (_locker) {
					try
					{
						...
						// Show the reader started messageLog once.
	...
	/**
	 * Logs the messageLog from HidBridge.
	 * @param m to log.
	 */
	private void Log(String m) {
//		LogHandler logHandler = LogHandler.getInstance();
//		logHandler.WriteMessage("HidBridge: " + messageLog, LogHandler.GetNormalColor());
		Log.d("debug",m);
